...
-- Run this on your psctech database in Azure Portal Query Editor
...
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID('institutions') AND type in (N'U'))
BEGIN
...
BEGIN
    ...
    BEGIN
        ALTER TABLE users ADD updated_at DATETIME2 DEFAULT GETUTCDATE();
...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID('user_profiles') AND type in (N'U'))
BEGIN
...
BEGIN TRY
    INSERT INTO institutions (name, type, email, phone, address, code, DatabaseName, ServerName)
