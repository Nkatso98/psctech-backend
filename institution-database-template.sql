-- PSC Tech Institution Database Template
-- This schema will be deployed to each institution's dedicated database
-- Version: 3.0.0 - Institution Template

-- =====================================================
-- INSTITUTION-SPECIFIC TABLES
-- =====================================================

-- Institution Details (Single record per database)
...
CREATE TABLE classes (
    ...
    CONSTRAINT UQ_class_academic_year UNIQUE (academic_year_id, code),
...
CREATE NONCLUSTERED INDEX IX_users_role ON users(role) INCLUDE (username, email, is_active);
CREATE NONCLUSTERED INDEX IX_users_email ON users(email) INCLUDE (username, role, is_active);

-- Academic performance indexes
CREATE NONCLUSTERED INDEX IX_classes_academic_year ON classes(academic_year_id) INCLUDE (grade_id, is_active);
CREATE NONCLUSTERED INDEX IX_students_class ON students(current_class_id) INCLUDE (enrollment_status);
...
CREATE NONCLUSTERED INDEX IX_announcements_published ON announcements(is_published, target_audience, published_at);
...
CREATE NONCLUSTERED INDEX IX_student_enrollment ON students(enrollment_status) INCLUDE (current_class_id, grade_level_id);
CREATE NONCLUSTERED INDEX IX_teacher_employment ON teachers(employment_status) INCLUDE (employee_number, department);
...
GROUP BY s.id, s.student_number, u.full_name, c.name, g.name;
...
-- Insert institution details (this will be populated when the database is created)
INSERT INTO institution_details (name, code, type, district, province, subscription_plan)
VALUES ('Institution Name', 'INST001', 'primary', 'District', 'Province', 'basic');

-- Insert sample academic year
INSERT INTO academic_years (name, start_date, end_date, is_current, status)
VALUES ('2024-2025', '2024-01-15', '2024-12-15', 1, 'active');
...
1. Complete data isolation per institution
2. Institution-specific schema
3. No cross-database dependencies
4. Independent user management
5. Local data operations only
...
Each institution database is completely isolated and contains only its own data.
*/
